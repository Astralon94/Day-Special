...
// Metodo condiviso della famiglia Zen (derivato da Zen-Store), adattato per Day-Special.
...
export function fetchBuffer(url, { timeoutMs = 15000, redirects = 5 } = {}) {
  return new Promise((res, rej) => {
    ...
    const req = getter(u, { headers: { 'User-Agent': 'Day-Special-Updater' } }, (r) => {
...
export async function checkUpdate(manifestUrl, appDir = APP_DIR) {
  ...
  // L'url del pacchetto può essere relativo al manifest (es. "day-special-1.0.0.json.gz"): lo risolvo.
  let download_url = '';
